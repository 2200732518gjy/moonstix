...
def category(root: Path, path: Path) -> str:
    ...
    if name.endswith("_test.mbt") or name.endswith("_wbtest.mbt"):
        return "tests"
    if relative.parts and relative.parts[0] == "cmd":
        return "examples"
...
def collect(root: Path) -> dict[str, LineCount]:
    totals = {name: LineCount() for name in ("core", "tests", "examples", "other")}
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"core effective: {totals['core'].effective}")
        print("core excludes tests, examples, cmd, and generated output")
